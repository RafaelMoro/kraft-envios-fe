...
| `dashboard/requests/[requestId]/page.tsx` | Standalone server page for the balance-request email deep link; owns its own cookie read and auth boundary (unauthenticated → redirect with return URL, non-admin → unauthorized panel, admin → detail). No `src/app/dashboard/layout.tsx`, so it inherits only the root layout — no dashboard chrome (`Aside`/mobile drawer). |
...
- The post-login return-URL pattern (`/dashboard/requests/[requestId]` email deep link) lives in exactly two call sites: `src/app/page.tsx` (server-side, reads `?redirect=`) and `LoginCard.tsx` (client-side, re-sanitizes the prop as defense in depth before `router.push`). Both call `sanitizeDashboardReturnUrl()` from `src/shared/utils/global.utils.ts`, which allowlists same-origin `/dashboard` paths and falls back to `/dashboard` otherwise. Do not apply this pattern to other routes without reason.
