...
export const QUOTE_SOURCES = ["GE", "TONE", "Pkk", "Mn"] as const;

export const QUOTE_SERVICE_TYPES = ["standard", "nextDay"] as const;
...
export const QuoteFormSchema: ObjectSchema<GetQuoteForm> = object().shape({
  originPostalCode: string()
    ...
    .matches(/^\d+$/, {
      ...
      message: "El código postal de origen solo puede contener dígitos",
    })
  ...
  destinationPostalCode: string()
    ...
    .matches(/^\d+$/, {
      excludeEmptyString: true,
      message: "El código postal de destino solo puede contener dígitos",
    })
  ...
  length: number()
    ...
    .required("El largo del paquete es requerido")
    .min(1, "El largo debe ser mayor que 0"),
  height: number()
    .transform((value, originalValue) =>
      originalValue === "" ? undefined : value,
    )
    .required("La altura del paquete es requerida")
    .min(1, "La altura debe ser mayor que 0"),
  width: number()
    .transform((value, originalValue) =>
      originalValue === "" ? undefined : value,
    )
    .required("El ancho del paquete es requerido")
