...
import {
  ...
  ADDRESS_GE_API_ENDPOINT,
  GET_ADDRESS_INFO_API_ENDPOINT,
} from "../constants/global.constants";
import {
  AddressExtraInfoGE,
  CreateAddressGEPayload,
  DeleteGEAdressResponse,
...
export const editAddressCb = async (data: CreateAddressPayload) => {
  try {
    const res: AxiosResponse<AddressAliasResponse> = await axios.put(
      ADDRESS_API_ENDPOINT,
      data,
    );
    return res?.data;
...
export const getAddressesCb = async (): Promise<Address[] | null> => {
  try {
    const res: AxiosResponse<GetAddressesResponse> =
      await axios.get(ADDRESS_API_ENDPOINT);
...
export const deleteAddressCb = async (data: DeleteAddressPayload) => {
  try {
    const res: AxiosResponse<AddressAliasResponse> = await axios.delete(
      ADDRESS_API_ENDPOINT,
      {
        data,
      },
...
export const saveAddressToLocalStorage = async (
  payload: CreateAddressGEPayload,
) => {
  try {
    ...
    addToLocalStorage({
      prop: PENDING_GE_ADDRESSES_KEY,
      newInfo: newPendingGEAddresses,
    });
  } catch (error) {
...
export const removeAddressFromLocalStorage = async (alias: string) => {
  try {
    const localStorage = getLocalStorageInfo();
    const pendingGEAddresses = localStorage[PENDING_GE_ADDRESSES_KEY] || [];
    const updatedAddresses = pendingGEAddresses.filter(
      (address: CreateAddressGEPayload) => address.alias !== alias,
    );
    addToLocalStorage({
      prop: PENDING_GE_ADDRESSES_KEY,
      newInfo: updatedAddresses,
    });
  } catch (error) {
...
export const formatAddressForDisplay = (
  ...
  const addressNumberInfo = information?.internalNumber
    ? `${information.externalNumber}, ${information.internalNumber}`
    : `${information.externalNumber}`;
