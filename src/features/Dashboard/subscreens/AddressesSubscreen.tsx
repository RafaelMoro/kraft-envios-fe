...
export const AddressesSubscreen = ({ userInfo }: AddressesSubscreenProps) => {
  ...
  const [pendingAddressesGE, setPendingAddressesGE] = useState<
    CreateAddressGEPayload[] | null
  >(null);
    ...
    <main className="w-full p-4">
      ...
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl font-bold text-center">
          Bienvenido {userInfo?.data?.user?.name}
        </h1>
        <p className="text-gray-600 dark:text-gray-400 text-center">
          Aquí puedes gestionar las direcciones que uses posteriormente para
          crear guías.
        </p>
          ...
          {addressesData &&
            ...
            !isError && (
              <div className="w-full md:col-span-2 lg:col-span-3 flex flex-col justify-center items-center gap-5">
                <Image
                  alt="No addresses available"
                  src="/empty-kraft-truck.webp"
                  width={1021}
                  height={597}
                ...
                <Button onClick={toggleModalManageAddress}>
                  Crear dirección
                </Button>
              </div>
            )}
      ...
      {pendingAddressesGE && pendingAddressesGE.length > 0 && (
        <section className="mt-20 flex flex-col gap-8">
          <h2 className="text-2xl font-bold text-center">
            Direcciones pendientes por crear en GE
          </h2>
