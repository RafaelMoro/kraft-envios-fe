...
| File | Source |
| --- | --- |
| `public/landing-hero-quotes.webp` | **In place.** 1200×1138, 34 KB (down from the 130 KB PNG extracted from manifest key `fcc5b1fc-57ff-4e9b-bd0e-64b02e699602`; the placeholder `.png` has been deleted). **One file only** — `next/image` generates the responsive `srcset`. Render with `next/image` (`.github/copilot-instructions.md` forbids mocking `next/image` in tests, so it must be a real import), `width={1200} height={1138}`, `priority`, and `sizes="(min-width: 1024px) 517px, min(560px, 100vw - 56px)"`. Cap the stacked mockup panel at `max-w-[560px] mx-auto` below `lg` so the image never renders wider than ~560 CSS px at any breakpoint — see epic Open Question X for the derivation. Alt text is in `docs/landing-copy-es.md` §1. |
