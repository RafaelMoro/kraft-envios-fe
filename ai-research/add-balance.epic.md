...
- Story 5, Email Deep Link To Admin Review: completed — last story of the epic. Implemented per `ai-planning/balance-addition/planning-email-deep-link-admin-review.story-5.md`: the single-request response/error types, route constants, copy, callback, shared timestamp formatter, and the open-redirect-safe return-URL sanitizer (Phase 1); the `GET /api/balance/requests/admin/[requestId]` BFF route with the defensive admin guard, preserving the flat `404 BAL_NF_001` and enveloped `403 Forbidden` verbatim (Phase 2); the standalone `BalanceAdminRequestDetail` surface (loading/error/not-found/pending/read-only/success-panel states, reusing `BalanceDecisionForm` unchanged) and `BalanceRequestUnauthorized` panel (Phase 3); the `src/app/dashboard/requests/[requestId]/page.tsx` server boundary (Phase 4); and the login return-URL plumbing (`src/app/page.tsx` reading `?redirect=`, `LoginCard` re-sanitizing before `router.push`) plus the `REPO_CONTEXT.md` update (Phase 5). No dashboard sidebar/shell was adopted, per the research's confirmed divergence from the comps. **Post-implementation revision:** manual QA found the Phase 4 page-level `getAccessToken()`/`redirect()` SSR guard intermittently misreading a genuinely valid session on a direct navigation (reproduced even from a fresh incognito login). The auth boundary moved from the server page to the client: `page.tsx` now renders `BalanceAdminRequestDetail` unconditionally, and that component reacts to its own data query's `400`/`403` from the BFF (redirect to login / render `BalanceRequestUnauthorized`) — the BFF's own fresh cookie read proved reliable where the page-render-time read did not. Full verification passed: `pnpm test` (96 suites, 1163 passed/3 pre-existing skipped plus the revised auth-boundary tests), `pnpm exec tsc --noEmit`, `pnpm lint`.
